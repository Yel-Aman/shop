...
class AllProductsView(View):
    def get(self,request):
        ...
        if params.get('search'):
            products = products.filter(Q(name__icontains = params['search'])|Q(desciption__icontains=params['search']))
...
def add_product(request):
    if request.method == 'POST':
        form = ProductForm(request.POST) #dannyeperehodyatvjsonformat,pythonperevoditihvdict
        if form.is_valid():
            form.save()
